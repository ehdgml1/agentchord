"""MCP to AgentChord tool adapter.

This module bridges MCP tools into AgentChord's tool system,
...
from agentchord.protocols.mcp.types import MCPTool
...
def mcp_tool_to_tool(mcp_tool: MCPTool, mcp_client: MCPClient) -> Tool:
    """Convert an MCPTool to an AgentChord Tool.
...
def mcp_tools_to_tools(mcp_tools: list[MCPTool], mcp_client: MCPClient) -> list[Tool]:
    """Batch convert multiple MCPTools to AgentChord Tools.
    ...
    Returns:
        List of AgentChord Tool objects.
    """
...
async def register_mcp_tools(mcp_client: MCPClient, executor: ToolExecutor) -> list[str]:
    ...
    converts them to AgentChord Tools, and registers them with the executor.
