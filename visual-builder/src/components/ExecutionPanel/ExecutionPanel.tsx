...
export const ExecutionPanel = memo(function ExecutionPanel({ className }: ExecutionPanelProps) {
  ...
  const { events, isStreaming } = useExecutionProgress(
    displayExecution?.status === 'running' ? selectedId : null
  );
  ...
  return (
    <aside className={cn('w-80 border-l bg-background flex flex-col', className)} aria-label="Execution history panel">
        ...
        <div className="mb-3 flex items-center gap-2">
          ...
          {displayExecution && (
            <span className="text-xs font-medium">
              {displayExecution.nodeExecutions.length} node
              {displayExecution.nodeExecutions.length !== 1 ? 's' : ''}
            </span>
          )}
        </div>
