"""Tool factories for multi-agent delegation, shared context access, and peer consultation.

This module provides factory functions that create Tool instances which enable
a coordinator agent to delegate tasks to team members, access shared state,
and allow worker agents to consult peers during multi-agent orchestration.
...
Key Functions:
    ...
    create_consult_tools: Create tools for peer-to-peer agent consultation

Example:
    >>> from agentchord.orchestration import (
...
def _sanitize_tool_name(raw: str) -> str:
    """Sanitize tool name for LLM API compatibility (Gemini etc).

    Replaces non-ASCII and invalid characters with underscores.
    Falls back to hash suffix if result would be empty.
    """
    sanitized = re.sub(r'[^a-zA-Z0-9_.\:\-]', '_', raw)
    sanitized = re.sub(r'_+', '_', sanitized)  # collapse multiple underscores
    sanitized = sanitized.strip('_')
    if not sanitized:
        # Fully non-ASCII name — use hash as fallback
        sanitized = f"agent_{abs(hash(raw)) % 100000}"
    return sanitized[:64]
...
def create_delegation_tools(
    ...
    for name, agent, member_info in members:
        ...
        tool = Tool(
            name=f"delegate_to_{_sanitize_tool_name(name)}",
            description=(
...
def create_consult_tools(
    peers: list[tuple[str, Any, TeamMember | None]],
    current_agent_name: str,
    message_bus: MessageBus | None = None,
    on_consult: Any | None = None,
    max_depth: int = 1,
    _current_depth: int = 0,
) -> list[Tool]:
    """Create consult tools for peer-to-peer agent communication.

    Each tool allows a worker agent to ask a peer agent a question
    during its own execution. The peer runs the question and returns
    a response string.

    Args:
        peers: List of (name, agent, member_info) tuples.
        current_agent_name: Name of the agent receiving these tools (excluded from peers).
        message_bus: Optional message bus for recording consult messages.
        on_consult: Optional async callback ``(agent_name, result) -> None``.
        max_depth: Maximum consult chain depth.
        _current_depth: Internal depth counter (do not set manually).

    Returns:
        List of Tool instances, one per peer (excluding self).
    """
    ...
    for name, agent, member_info in peers:
        if name == current_agent_name:
            continue  # Can't consult yourself
        ...
        async def _consult(
            ...
            if on_consult is not None:
                await on_consult(_name, result)

            return result.output

        tool = Tool(
            name=f"consult_{_sanitize_tool_name(name)}",
            description=(
                f"Ask {name} ({role_str}) a question.{capabilities_str} "
                f"Use when you need verification, clarification, or expertise from {name}."
            ),
            parameters=[
                ToolParameter(
                    name="message",
                    type="string",
                    description="Your question or request for the peer agent",
                    required=True,
                ),
            ],
            func=_consult,
