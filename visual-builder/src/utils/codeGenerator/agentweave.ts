...
export class AgentWeaveGenerator implements CodeGenerator {
  ...
  private generateImports(hasAgents: boolean, hasMultipleAgents: boolean): string {
    if (!hasAgents) {
      return 'from agentweave import Agent';
    }

    if (hasMultipleAgents) {
      return 'from agentweave import Agent, Workflow';
    }
  ...
  private generateComplexImports(nodes: WorkflowNode[], hasAsyncNodes: boolean): string {
    ...
    if (hasMultiAgent) {
      imports.push('from agentweave import Agent, AgentTeam');
    } else if (hasAgents) {
      imports.push('from agentweave import Agent');
    } else {
    ...
    if (hasMCPTools) {
      imports.push('# from agentweave.protocols.mcp import MCPClient');
    }
