"""Unit tests for AgentWeave telemetry module."""
...
from agentweave.telemetry import AgentWeaveTracer, AgentWeaveMetrics, setup_telemetry, get_tracer
from agentweave.telemetry.tracer import (
    _NoOpSpan,
    ATTR_AGENT_NAME,
    ATTR_AGENT_ROLE,
    ATTR_AGENT_MODEL,
...
class TestSemanticAttributes:
    ...
    def test_attribute_constants_exist(self) -> None:
        ...
        assert ATTR_AGENT_NAME == "agentweave.agent.name"
        assert ATTR_AGENT_ROLE == "agentweave.agent.role"
        assert ATTR_AGENT_MODEL == "agentweave.agent.model"
        assert ATTR_PROVIDER == "agentweave.provider"
        assert ATTR_TOOL_NAME == "agentweave.tool.name"
        assert ATTR_TOOL_SUCCESS == "agentweave.tool.success"
        assert ATTR_WORKFLOW_NAME == "agentweave.workflow.name"
        assert ATTR_WORKFLOW_FLOW == "agentweave.workflow.flow"
        assert ATTR_TOKENS_PROMPT == "agentweave.tokens.prompt"
        assert ATTR_TOKENS_COMPLETION == "agentweave.tokens.completion"
        assert ATTR_TOKENS_TOTAL == "agentweave.tokens.total"
        assert ATTR_COST_USD == "agentweave.cost.usd"
        assert ATTR_DURATION_MS == "agentweave.duration_ms"
        assert ATTR_TOOL_ROUNDS == "agentweave.tool_rounds"


class TestAgentWeaveTracerWithoutOTEL:
    """Tests for AgentWeaveTracer when OpenTelemetry is not available."""

    @patch("agentweave.telemetry.tracer.HAS_OTEL", False)
...
class TestAgentWeaveTracerWithOTEL:
    """Tests for AgentWeaveTracer when OpenTelemetry is available.
    ...
    @pytest.fixture(scope="class")
    def shared_tracer(self, shared_exporter: "InMemorySpanExporter") -> AgentWeaveTracer:
        """Create a shared tracer for all tests in this class."""
        # Reset module-level tracer first
        import agentweave.telemetry.tracer as tracer_module
        tracer_module._tracer = None
        return setup_telemetry(service_name="test-telemetry", exporter=shared_exporter)

    def test_tracer_enabled_when_otel_available(self, shared_tracer: AgentWeaveTracer) -> None:
    ...
    def test_agent_span_creates_span_with_attributes(
        self, shared_tracer: AgentWeaveTracer, shared_exporter: "InMemorySpanExporter"
    ) -> None:
    ...
    def test_workflow_span_creates_span_with_attributes(
        ...
        assert span_data.attributes["agentweave.workflow.agent_count"] == 3
...
class TestAgentWeaveMetricsWithoutOTEL:
    """Tests for AgentWeaveMetrics when OpenTelemetry is not available."""

    @patch("agentweave.telemetry.metrics.HAS_OTEL", False)
    def test_record_agent_run_doesnt_raise(self) -> None:
        """record_agent_run should not raise when OTEL disabled."""
        metrics = AgentWeaveMetrics()
...
@pytest.mark.skipif(not HAS_OTEL, reason="OpenTelemetry not installed")
class TestAgentWeaveMetricsWithOTEL:
    """Tests for AgentWeaveMetrics when OpenTelemetry is available."""
    ...
    def test_record_agent_run_tracks_metrics(self) -> None:
        ...
        with patch("agentweave.telemetry.metrics.metrics.get_meter") as mock_get_meter:
...
class TestSetupTelemetry:
    ...
    def test_setup_raises_import_error_when_otel_unavailable(self) -> None:
        ...
        assert "pip install agentweave[telemetry]" in str(exc_info.value)

    @pytest.mark.skipif(not HAS_OTEL, reason="OpenTelemetry not installed")
    def test_setup_returns_tracer(self) -> None:
        """setup_telemetry should return AgentWeaveTracer instance."""
        ...
        assert isinstance(tracer, AgentWeaveTracer)
...
class TestGetTracer:
    ...
    def test_get_tracer_returns_tracer(self) -> None:
        """get_tracer should return AgentWeaveTracer."""
        tracer = get_tracer()
        assert isinstance(tracer, AgentWeaveTracer)
