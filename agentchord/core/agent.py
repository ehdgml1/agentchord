"""Agent implementation - the core component of AgentChord.
...
from agentchord.errors.exceptions import AgentExecutionError, ModelNotFoundError
...
if TYPE_CHECKING:
    ...
    from agentchord.memory.base import BaseMemory
    from agentchord.protocols.mcp.client import MCPClient
...
class Agent:
    ...
    Agents are the fundamental building blocks of AgentChord. Each agent
    ...
    async def setup_mcp(self) -> list[str]:
        ...
        Converts MCP tools to AgentChord tools and registers them.
        ...
        from agentchord.protocols.mcp.adapter import register_mcp_tools
        from agentchord.tools.executor import ToolExecutor
...
- Provide actionable responses when possible"""
    ...
    async def run(
        ...
        if self._tool_executor:
            ...
            if provider_name == "openai":
                ...
                # Gemini, Ollama, and other OpenAI-compatible providers
                kwargs["tools"] = self._tool_executor.to_openai_tools()
        ...
        try:
            ...
            if response is not None and not response.content and tools_were_used and loop_broke_naturally:
                synth_kwargs = {k: v for k, v in kwargs.items() if k != "tools"}
                await self._emit_callback("llm_start", model=self.model)
                response = await self._execute_llm(messages, **synth_kwargs)
                total_prompt_tokens += response.usage.prompt_tokens
                total_completion_tokens += response.usage.completion_tokens
                await self._emit_callback(
                    "llm_end", model=self.model, tokens=response.usage.total_tokens
        ...
        try:
            ...
            for _round in range(max_tool_rounds):
                ...
                if self._tool_executor:
                    ...
                    if not response.content and tools_were_used and loop_broke_naturally:
                        synth_kwargs = {k: v for k, v in kwargs.items() if k != "tools"}
                        await self._emit_callback("llm_start", model=self.model)
    ...
    @asynccontextmanager
    async def temporary_tools(self, tools: "list[Tool]"):
        """Temporarily add tools to this agent, restoring original state on exit.

        This is the public API for strategies to inject delegation tools
        without accessing private fields.

        Args:
            tools: Tools to temporarily add.

        Yields:
            self (the agent with tools injected).
        """
        from agentchord.tools.executor import ToolExecutor

        had_executor = self._tool_executor is not None
        if self._tool_executor is None:
            self._tool_executor = ToolExecutor()

        original_tool_names = set(self._tool_executor.tool_names)
        added_names = [t.name for t in tools]
    ...
    @asynccontextmanager
    async def with_extended_prompt(self, extension: str):
        """Temporarily extend the system prompt, restoring on exit.

        Args:
            extension: Text to append to the current system prompt.

        Yields:
            self (the agent with extended prompt).
        """
        original = self._system_prompt
        if original:
            self._system_prompt = f"{original}\n\n{extension}"
        else:
            self._system_prompt = extension
        try:
            yield self
        finally:
            self._system_prompt = original
