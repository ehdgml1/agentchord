...
class MapReduceStrategy(BaseStrategy):
    ...
    async def execute(
        ...
        for i, result_or_exc in enumerate(results):
            ...
            if ctx.shared_context is not None:
                await ctx.shared_context.set(
                    f"{name}_map", result.output, agent=name
                )
        ...
        if ctx.callbacks:
            await ctx.callbacks.emit(
                CallbackEvent.REDUCE_PHASE_START,
                reducer=reducer_name,
                partial_count=len(partial_outputs),
                strategy="map_reduce",
            )
        ...
        if message_bus:
            await message_bus.send(AgentMessage(
                sender="system",
                recipient=reducer_name,
                message_type=MessageType.TASK,
                content="Reduce/merge results",
        ...
        if ctx.enable_consult:
            other_agents = [
                ...
                for n, a in agent_list if n != reducer_name
            ]
            consult_tools = create_consult_tools(
                peers=other_agents,
                current_agent_name=reducer_name,
                message_bus=message_bus,
                max_depth=ctx.max_consult_depth,
            )
            async with reducer_agent.temporary_tools(consult_tools):
        ...
        if message_bus:
            await message_bus.send(AgentMessage(
                ...
                recipient="system",
                message_type=MessageType.RESULT,
                content=merge_result.output,
                metadata={"phase": "reduce"},
            ))

        if ctx.shared_context is not None:
            await ctx.shared_context.set("reduce_result", merge_result.output, agent=reducer_name)
